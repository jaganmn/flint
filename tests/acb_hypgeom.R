...
## http://dlmf.nist.gov/15.4.E17
          ...
          z. <- z[i.]; a. <- a[i.]
          all.equal(as.complex(h2f1(a., 0.5 + a., 1 + 2 * a., z.)),
...
## http://dlmf.nist.gov/15.4.E18
          ...
          i. <- if (debugging) TRUE else -10L
          z. <- z[i.]; a. <- a[i.]
          all.equal(as.complex(h2f1(a., 0.5 + a., 2 * a., z.)),
                    (0.5 + 0.5 * sqrt(1 - z.))^(1 - 2 * a.)/sqrt(1 - z.))
          },
## http://dlmf.nist.gov/15.4.E19
          all.equal(as.complex(h2f1(a + 1, b, a, z)),
                    (1 - (1 - b/a) * z) * (1 - z)^(-1 - b)),
## http://dlmf.nist.gov/15.4.E20
          {
          if (tryCatch({ lgamma(1i); FALSE }, error = function(e) TRUE))
              TRUE
          else {
          i. <- Re(c - a - b) > 0
          z. <- z[i.]; a. <- a[i.]; b. <- b[i.]; c. <- c[i.]
          all.equal(as.complex(h2f1(a, b, c, 1)),
                    exp(lgamma(c) + lgamma(c - a - b) - lgamma(c - a) - lgamma(c - b)))
          }
          })
