...
SEXP R_flint_acb_hypgeom_2f1(SEXP s_res, SEXP s_a, SEXP s_b, SEXP s_c, SEXP s_z, SEXP s_flags, SEXP s_prec)
{
	...
	unsigned long long int i,
		...
		nb = _R_flint_get_length(s_b),
		nc = _R_flint_get_length(s_c),
	...
	acb_ptr res = (acb_ptr) flint_calloc(n, sizeof(acb_t)),
		...
		b = (acb_ptr) _R_flint_get_x(s_b),
		c = (acb_ptr) _R_flint_get_x(s_c),
...
SEXP R_flint_acb_hypgeom_2f1_continuation(SEXP s_res0, SEXP s_res1, SEXP s_a, SEXP s_b, SEXP s_c, SEXP s_z0, SEXP s_z1, SEXP s_f0, SEXP s_f1, SEXP s_prec)
{
	unsigned long long int i,
		...
		nb = _R_flint_get_length(s_b),
		nc = _R_flint_get_length(s_c),
		nz0 = _R_flint_get_length(s_z0),
		nz1 = _R_flint_get_length(s_z1),
		nf0 = _R_flint_get_length(s_f0),
		nf1 = _R_flint_get_length(s_f1),
		nprec = _R_flint_get_length(s_prec),
		n = RECYCLE5(na, nb, nc, RECYCLE4(nz0, nz1, nf0, nf1), nprec);
	_R_flint_set_length(s_res0, n);
	_R_flint_set_length(s_res1, n);
	...
	acb_ptr res0 = (acb_ptr) flint_calloc(n, sizeof(acb_t)),
		...
		b = (acb_ptr) _R_flint_get_x(s_b),
		c = (acb_ptr) _R_flint_get_x(s_c),
		z0 = (acb_ptr) _R_flint_get_x(s_z0),
		z1 = (acb_ptr) _R_flint_get_x(s_z1),
		f0 = (acb_ptr) _R_flint_get_x(s_f0),
		f1 = (acb_ptr) _R_flint_get_x(s_f1);
	slong *prec = (slong *) _R_flint_get_x(s_prec);
	_R_flint_set_x(s_res0, res0, (R_CFinalizer_t) &R_flint_acb_finalize);
	_R_flint_set_x(s_res1, res1, (R_CFinalizer_t) &R_flint_acb_finalize);
